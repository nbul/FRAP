...
for g=1:numel(files)
    ...
    FRAP1 = zeros(seriesCount/2,1);
    Control = zeros(seriesCount/2,1);
    BG = zeros(seriesCount/2,1);
    
    for q=1:seriesCount/2
        Image_time = Series{q*2-1,1};
    ...
    FRAP_norm = FRAP_norm(Pre_bleach_N+1:seriesCount/2);
    
    FRAP_final(:,g) = (FRAP_norm-FRAP_norm(1))/(1-FRAP_norm(1));
